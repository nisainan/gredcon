...
//根据decoder的Sync.pool进行 内存单元的获取
func NewReaderSizeWithSyncPool(rd io.Reader, pool *sync.Pool, size int) *Reader {
	buffer, ok := pool.Get().(*[]byte)
	if !ok || buffer == nil {
		return &Reader{rd: rd, buf: make([]byte, size)}
	}
	return &Reader{rd: rd, buf: *buffer}
}

//归还内存给decoder的sync.pool
func (b *Reader) ReleaseSyncPoolBuffer(pool *sync.Pool) {
	pool.Put(&b.buf)
}
