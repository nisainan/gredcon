...
import (
	...
	"redcon/pkg/resparse/bufio2"
	"redcon/pkg/resparse/errors"
)
...
func init() {
	bufferSyncPool = &sync.Pool{
		New: func() interface{} {
			data := make([]byte, SyncPoolBufferSize) //sync.pool 每个元素 1024长度 []byte
			return &data
		},
	}
}

//创建resp 解码器：并注入sync.pool
func NewDecoderWithPoolBuffer(r io.Reader) *Decoder {
	return NewDecoderBuffer(bufio2.NewReaderSizeWithSyncPool(r, bufferSyncPool, SyncPoolBufferSize))
}

func (d *Decoder) ReleaseDecoderPoolBuffer() {
	d.br.ReleaseSyncPoolBuffer(bufferSyncPool)
}
